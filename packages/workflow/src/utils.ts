...
/**
 * Flattens a deeply nested JSON object using dot notation for keys.
 * Arrays are processed with numeric indices in the key notation.
 *
 * @param obj - The JSON object to flatten
 * @param prefix - Internal parameter for recursion (the current key prefix)
 ...
 *   f: [{ g: 4 }, { h: 5 }]
 * })
 * // Returns: { "a": 1, "b.c": 2, "b.d.e": 3, "f.0.g": 4, "f.1.h": 5 }
 */
    ...
    if (Array.isArray(obj)) {
        if (obj.length === 0) {
            ...
            obj.forEach((item, index) => {
                ...
                const flattened = flattenJSON(item, key);
